...
public:
    ...
    void stateCallback(const gazebo_msgs::ModelStates::ConstPtr& msg) {
        try {
            ...
            double xdes = 0, ydes = 9, zdes = 1.25;
            ...
            if ( ry <= -4)
             {
                ...
                double crx4 = - 1.5*triangularWave(time  + 1 , 60) + 3 ;
                MX ob4 = abs(X(0, k + 1) - (crx4 - 0.5)) + 
                         abs(X(0, k + 1) - (crx4 + 0.5)) + 
                         abs(X(1, k + 1) + 4.75) + 
                         abs(X(1, k + 1) + 5.25) + 
                         abs(X(2, k + 1) ) + 
                         abs(X(2, k + 1) - 1.5);
            ...
            else if ( -4 < ry && ry <= 0)
            {
               for (int k = 0; k < N; ++k) {
                double time = current_time.toSec() + k * T;
                double crx1 = triangularWave(time, 40) - 2 ;
                MX ob1 = abs(X(0, k + 1) - (crx1 - 0.5)) + 
                         abs(X(0, k + 1) - (crx1 + 0.5)) + 
                         abs(X(1, k + 1) + 0.75) + 
                         abs(X(1, k + 1) + 1.25) + 
                         abs(X(2, k + 1) ) + 
                         abs(X(2, k + 1) - 1.5);
            ...
            else if ( 0 < ry && ry < 4) 
            {
                for (int k = 0; k < N; ++k) {
                double time = current_time.toSec() + k * T;
                double crx2 = - triangularWave(time, 40) + 2 ;
                MX ob2 = abs(X(0, k + 1) - (crx2 - 0.5)) + 
                         abs(X(0, k + 1) - (crx2 + 0.5)) + 
                         abs(X(1, k + 1) - 2.75) + 
                         abs(X(1, k + 1) - 3.25) + 
                         abs(X(2, k + 1) ) + 
                         abs(X(2, k + 1) - 1.5);
                ...
                // Ensure the sum of the absolute differences is greater than 5.5
                opti.subject_to(ob2 >= 3.3);
            }
            } 
            else{
                for (int k = 0; k < N; ++k) {
                double time = current_time.toSec() + k * T;
                double crx3 =  1.5*triangularWave(time + 8 , 60) - 3 ;
                MX ob3 = abs(X(0, k + 1) - (crx3 - 0.5)) + 
                         abs(X(0, k + 1) - (crx3 + 0.5)) + 
                         abs(X(1, k + 1) - 5.75) + 
                         abs(X(1, k + 1) - 6.25) + 
                         abs(X(2, k + 1) ) + 
                         abs(X(2, k + 1) - 1.5);
                opti.subject_to(ob3 >= 3.3);
            }
            }
            // ---- path constraints -----------
            // Initial constraints
            ...
            outfile.open("dev_ws/src/dynacube/data/planningdynacube.txt", std::ios_base::app); // Append mode
