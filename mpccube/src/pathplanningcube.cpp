...
public:
    ...
    void stateCallback(const gazebo_msgs::ModelStates::ConstPtr& msg) {
        try {
            ...
            MX ob1 = abs(X(0,all) + 0.673) + abs(X(0,all) + 2.473) + abs(X(1,all) - 1.25) + abs(X(1,all) - 3.35) + abs(X(2,all) ) + abs(X(2,all) - 1.8); 
            MX ob2 = abs(X(0,all) + 0.49) + abs(X(0,all) + 2.49) + abs(X(1,all) - 4.12) + abs(X(1,all) - 6.32) + abs(X(2,all) ) + abs(X(2,all) - 1.5);
            MX ob3 = abs(X(0,all) - 0.4) + abs(X(0,all) - 2.5) + abs(X(1,all) - 3.45) + abs(X(1,all) - 5.35) + abs(X(2,all) ) + abs(X(2,all) - 1.8);
            MX ob4 = abs(X(0,all) - 2.25) + abs(X(0,all) - 6.15) + abs(X(1,all) - 0.2) + abs(X(1,all) - 1) + abs(X(2,all) ) + abs(X(2,all) - 2.6); 
            ...
            opti.subject_to(6.0 <= ob1);
            opti.subject_to(6.0 <= ob2);
            opti.subject_to(6.1 <= ob3);
            opti.subject_to(7.6 <= ob4);
            Dict ipopt_options;
            ...
            ipopt_options["ipopt.tol"] = 1e-2;        // Set tolerance to 1e-3
            ...
            state_msg.data[0] = static_cast<double>(X_sol(0, 15).scalar()); // X position
            state_msg.data[1] = static_cast<double>(X_sol(1, 15).scalar()); // Y position
            state_msg.data[2] = static_cast<double>(X_sol(2, 15).scalar()); // Z position
            traj_pub_.publish(state_msg);

            // Save the state to a file along with the current time
            std::ofstream outfile;
            outfile.open("dev_ws/src/mpccube/data/planningcube.txt", std::ios_base::app); // Append mode
