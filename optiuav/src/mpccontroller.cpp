...
public:
    ...
    void stateCallback(const gazebo_msgs::ModelStates::ConstPtr& msg) {
        try {
            ...
            outfilex.open("dev_ws/src/optiuav/data/positionuav.txt", std::ios_base::app); // Append mode
            outfilex << "Time: " << current_time.toSec() << ", x: " << rx << ", y: " << ry << ", z: " << rz << ", phi: " << rphi << ", theta: " << rtheta << ", psi: " << rpsi << "\n";
            outfilex.close();

            std::ofstream outfileu;
            outfileu.open("dev_ws/src/optiuav/data/controluav.txt", std::ios_base::app); // Append mode
