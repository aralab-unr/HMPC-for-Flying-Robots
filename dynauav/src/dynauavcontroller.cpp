...
#include "std_srvs/Empty.h"  // Add this header for Empty service
...
// Global parameters
const double m = 2.11, g = 9.81;
const double Ixx = 0.054, Iyy = 0.043, Izz = 0.0708;
double kpx = 0.125, kdx = 0.1, kpy = 0.125, kdy = 0.1;
...
public:
    ...
    void stateCallback(const gazebo_msgs::ModelStates::ConstPtr& msg) {
        try {
            ...
            outfilev.open("dev_ws/src/dynauav/data/velodynauav.txt", std::ios_base::app); // Append mode
            outfilev << "Time: " << current_time.toSec() << ", vx: " << rvx << ", vy: " << rvy << ", vz: " << rvz << ", vphi: " << rphidot << ", vtheta: " << rthetadot << ", vpsi: " << rpsidot << "\n";
            outfilev.close();
            ...
            // Input constraints
            opti.subject_to(18 <= U(0, all) <= 22); 
            opti.subject_to(-0.05 <= U(1, all) <= 0.05);  
            ...
            double Con1 = static_cast<double>(U_sol(0, 0).scalar());
            double Con2 = static_cast<double>(U_sol(1, 0).scalar());
            double Con3 = static_cast<double>(U_sol(2, 0).scalar());
            double Con4 = static_cast<double>(U_sol(3, 0).scalar());

            double kt = 0.00025, kd = 0.000075, l = 0.159;
            double w12, w22, w32, w42;

            w12 = (Con3 * kd + Con2 * kd + Con1 * kd * l - Con4 * kt * l) / (4 * kd * kt * l);
            w22 = (Con3 * kd - Con2 * kd + Con1 * kd * l + Con4 * kt * l) / (4 * kd * kt * l);
            w32 = (-Con3 * kd + Con2 * kd + Con1 * kd * l + Con4 * kt * l) / (4 * kd * kt * l);
            w42 = (-Con3 * kd - Con2 * kd + Con1 * kd * l - Con4 * kt * l) / (4 * kd * kt * l);
            ...
            outfilex.open("dev_ws/src/dynauav/data/positiondynauav.txt", std::ios_base::app); // Append mode
            outfilex << "Time: " << current_time.toSec() << ", x: " << rx << ", y: " << ry << ", z: " << rz << ", phi: " << rphi << ", theta: " << rtheta << ", psi: " << rpsi << "\n";
            outfilex.close();

            std::ofstream outfileu;
            outfileu.open("dev_ws/src/dynauav/data/controldynauav.txt", std::ios_base::app); // Append mode
            outfileu << "Time: " << current_time.toSec() << ", U1: " << Con1 << ", U2: " << Con2 << ", U3: " << Con3 << ", U4: " << Con4 << "\n";
            outfileu.close();

            std_msgs::Float64MultiArray float64_array_msg;
            float64_array_msg.data = {w1, w2, w3, w4};
...
int main(int argc, char** argv) {
    ...
    std_srvs::Empty reset_sim;
    if (ros::service::call("/gazebo/reset_simulation", reset_sim)) {
        ROS_INFO("Gazebo simulation time reset.");
    } else {
        ROS_ERROR("Failed to reset Gazebo simulation time.");
    }
